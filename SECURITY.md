...
Enquanto existir apenas a linha `1.1.x`, apenas a versão mais recente dessa linha recebe correções de segurança. Builds locais, forks e artefactos que não tenham sido publicados em [GitHub Releases](https://github.com/p-darksy-r/LocalNetworkScanner/releases) não são considerados releases oficiais.
...
O histórico local fica, por predefinição, em `%LOCALAPPDATA%\LocalNetworkScanner\snapshots`. Exportações JSON, CSV e HTML são gravadas no caminho escolhido pelo utilizador. Estes ficheiros podem conter um mapa sensível da rede e não são cifrados pela aplicação.
