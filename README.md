...
O workflow versionado atualmente implementa apenas o backend **Microsoft Artifact Signing por OIDC**. Esse backend não está configurado neste repositório e não deve ser confundido com a avaliação da SignPath Foundation, que não prosseguirá no âmbito gratuito após a orientação recebida. O repositório não contém transporte por GitHub Actions artifact, ação de submissão ou credenciais SignPath; uma eventual subscrição regular exigiria uma integração nova, configuração fornecida pelo serviço e revisão explícita. Em qualquer backend aceite, a chave privada permanece fora do repositório.
